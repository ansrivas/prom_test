...
use crate::error::{
    ...
    TableNotFoundSnafu, TimeIndexNotFoundSnafu, UnexpectedPlanExprSnafu, UnexpectedTokenSnafu,
...
impl PromPlanner {
    pub async fn stmt_to_plan(session_ctx: SessionContext, stmt: EvalStmt) -> Result<LogicalPlan> {
    ...
    async fn create_table_scan_plan(
        ...
        let result = LogicalPlanBuilder::scan_with_filters(
            table_ref,
    ...
    async fn setup_context(&mut self) -> Result<()> {
        ...
        let time_index = "_timestamp".to_string();
        ...
        let tags = table
            ...
            .iter()
            .filter_map(|f| {
                if f.name().to_string() != "_timestamp".to_string()
